...
| 3        | `/usr/lib/systemd/system/` | 发行版或软件包自带 | 软件安装时自动放置的原始配置          |

> 说明：有些发行版会把 `/lib/systemd/system/` 作为 `/usr/lib/systemd/system/` 的兼容符号链接，因此二者常被等价对待。

### 4.1 用户级（User）单元文件位置（`systemctl --user`）

当你使用 `systemctl --user` 管理“当前用户”的服务时，systemd 会搜索用户级 Unit 文件，优先级由高到低：

| 优先级   | 存放路径                      | 描述                         |
| -------- | ----------------------------- | ---------------------------- |
| 1 (最高) | `~/.config/systemd/user/`    | 用户自己目录的自定义单元      |
| 2        | `/etc/systemd/user/`          | 管理员为用户统一提供的配置    |
| 3        | `/run/user/$UID/systemd/user/` | 运行时生成的单元（随会话变化） |
| 4        | `/usr/lib/systemd/user/`      | 发行版或软件包自带的默认单元    |
